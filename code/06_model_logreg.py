...
def build_logreg_pipeline(preprocess, penalty='elasticnet', class_weight=None, random_seed=42):
    '''
    Builds a Logistic Regression model pipeline.
    ...
    class_weight (dict or str, optional): Class weights for Logistic Regression. Default is None.
    random_seed (int, optional): Random seed for reproducibility. Default is 42.

    Returns:
    Pipeline: A sklearn Pipeline with preprocessing and Logistic Regression.
    '''
...
def tune_logreg_model(pipe, params, cv, Xtrain, ytrain, n_iter=50, scoring='roc_auc', random_seed=42, n_jobs=-1):
    '''
...
def run_logreg_model(preprocess, cv, Xtrain, ytrain, penalty='elasticnet', class_weight=None, random_seed=42, n_iter=50, scoring='roc_auc', n_jobs=-1):
    '''
